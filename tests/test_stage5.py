...
class TestRoleAssignment:
    ...
    def test_developer_creates_develops_edge_to_system(self):
        ...
        # 行为演进：develops 边的 subject 现在是 role-actor 节点
        # （entity_type=AIDeveloper、name=stakeholder 名、hash id），
        # 不再直接使用原始 Stakeholder 节点 id
        assert develops_edges[0].subject_name == "OpenAI"
        developer_ids = {
            n.id for n in subgraph.nodes
            if n.entity_type == OntologyClass.AI_DEVELOPER and n.name == "OpenAI"
        }
        assert develops_edges[0].subject_id in developer_ids
    ...
    def test_provider_creates_provides_edge(self):
        ...
        # 行为演进：subject 是 role-actor 节点（AIProvider 类型），不再是 ent_Google
        assert provides_edges[0].subject_name == "Google"
        provider_ids = {
            n.id for n in subgraph.nodes
            if n.entity_type == OntologyClass.AI_PROVIDER and n.name == "Google"
        }
        assert provides_edges[0].subject_id in provider_ids
    ...
    def test_deployer_creates_deploys_edge(self):
        ...
        # 行为演进：subject 是 role-actor 节点（AIDeployer 类型），不再是 ent_Acme Corp
        assert deploys_edges[0].subject_name == "Acme Corp"
        deployer_ids = {
            n.id for n in subgraph.nodes
            if n.entity_type == OntologyClass.AI_DEPLOYER and n.name == "Acme Corp"
        }
        assert deploys_edges[0].subject_id in deployer_ids
    ...
    def test_regulator_investigates_incident(self):
        ...
        # 行为演进：同一监管者现在以两种节点表示——复用的 Regulator 实体节点
        # 和 role-actor 节点（Regulator 角色），_add_technical_role_edges
        # 会为每个 Regulator 类型节点各生成一条 investigates 边
        assert len(investigates) == 2
        assert all(e.object_id == "evt1" for e in investigates)
        assert all(e.subject_name == "FTC" for e in investigates)
    ...
    def test_regulator_enforces_regulation(self):
        ...
        # 行为演进：同上，监管者双重表示导致每个 Regulator 节点各一条 enforces 边
        assert len(enforces) == 2
        assert all(e.object_id == "ent_EU AI Act" for e in enforces)
        assert all(e.subject_name == "FTC" for e in enforces)
...
class TestInferenceNodes:
    def test_purpose_inference_creates_node(self):
        # 行为演进：Purpose 现在只建模为 AI 系统/模型的 capability，
        # 无 AISystem/GPAIModel 时会跳过推断，因此需要提供一个系统实体
        system = _make_entity("GPT-4", OntologyClass.AI_SYSTEM)
        state = _build_base_state()
        state["reused_entities"] = [system]
        state["inferences"] = {
    ...
    def test_inference_with_reasoning(self):
        # 行为演进：同上，purpose 推断需要存在 AI 系统/模型实体
        system = _make_entity("GPT-4", OntologyClass.AI_SYSTEM)
        state = _build_base_state()
        state["reused_entities"] = [system]
        state["inferences"] = {
