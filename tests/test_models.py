...
class TestOntologyClass:
    def test_total_class_count(self):
        # 行为演进：本体扩展后新增 GPAIModel/Misuse/AIComponent/Data，共 43 个类
        assert len(OntologyClass) == 43
...
class TestEvidenceItem:
    ...
    def test_create_full(self):
        # API 变更：mention_span 从裸字符串改为结构化 MentionSpan(text, start, end)
        item = EvidenceItem(
            evidence_id="ev1",
            evidence_sentence="OpenAI released GPT-4",
            source_doc_id="d1",
            mention_span=MentionSpan(text="OpenAI", start=0, end=6),
            confidence=0.9,
        )
        assert item.confidence == 0.9
        assert item.mention_span is not None
        assert item.mention_span.text == "OpenAI"
        assert item.mention_span.start == 0
        assert item.mention_span.end == 6
