...
class TestValidSubgraphPasses:
    def test_minimal_valid_subgraph(self):
        # Risk chain completeness requires hasRisk + (leadsTo or impacts).
        incident_id = "evt1"
...
class TestTypeConstraintViolations:
    def test_domain_violation_detected(self):
        ...
        # Also add valid hasRisk for chain completeness
        valid_edge = _make_edge(incident_id, RelationType.HAS_RISK, risk.id)
        leads_edge = _make_edge(incident_id, RelationType.HAS_CONSEQUENCE,
                                _make_entity("Cons", OntologyClass.CONSEQUENCE).id)

        state = _build_state_with_subgraph(
            [stakeholder, risk, _make_entity("Cons", OntologyClass.CONSEQUENCE)],
            [bad_edge, valid_edge, leads_edge],
        )
        result = validation_node(state)
        errors = result["validation_errors"]
        assert any("Domain violation" in e for e in errors)
    ...
    def test_range_violation_detected(self):
        ...
        # Also valid hasRisk for chain completeness
        risk = _make_entity("Risk", OntologyClass.RISK)
        valid_edge = _make_edge(incident_id, RelationType.HAS_RISK, risk.id)
        leads_edge = _make_edge(incident_id, RelationType.HAS_CONSEQUENCE,
                                _make_entity("Cons", OntologyClass.CONSEQUENCE).id)
        ...
        state = _build_state_with_subgraph(
            [risk_source, stakeholder, risk, _make_entity("Cons", OntologyClass.CONSEQUENCE)],
            [bad_edge, valid_edge, leads_edge],
        )
        result = validation_node(state)
        errors = result["validation_errors"]
        assert any("Range violation" in e for e in errors)
...
class TestRiskChainCompleteness:
    ...
    def test_risk_control_without_mitigates_fails(self):
        ...
        risk = _make_entity("Risk", OntologyClass.RISK)
        consequence = _make_entity("Cons", OntologyClass.CONSEQUENCE)
        rc = _make_entity("Audit", OntologyClass.RISK_CONTROL)

        # The RC node must appear as an edge's object_id to be detected
        # by validate_risk_chain_completeness (which checks object_type).
        # Add an edge that targets the RC but is NOT a mitigates predicate.
        edges = [
            _make_edge(incident_id, RelationType.HAS_RISK, risk.id),
            _make_edge(risk.id, RelationType.LEADS_TO, consequence.id),
            _make_edge(incident_id, RelationType.HAS_CONSEQUENCE, rc.id),
            # No mitigates edge for RiskControl
        ]
        state = _build_state_with_subgraph([risk, consequence, rc], edges)
        result = validation_node(state)
        errors = result["validation_errors"]
...
class TestValidationNodeMetadata:
    def _make_valid_state(self, event_id: str = "evt1") -> PipelineState:
        ...
        return _build_state_with_subgraph([risk, consequence], edges, event_id)
