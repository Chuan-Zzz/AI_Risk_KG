...
class TestValidateRiskChainCompleteness:
    ...
    def test_missing_consequence_and_impact_fails(self, ontology):
        ...
        errors = ontology.validate_risk_chain_completeness(edges)
        assert len(errors) >= 1
        # 行为演进：链式校验收紧，缺 Consequence/Impact 时首先报缺 Consequence
        assert any("at least one Consequence" in e for e in errors)

    def test_risk_with_consequence_requires_impact(self, ontology):
        # 行为演进：risk→consequence 之后链还必须延伸到 Impact 才算完整
        edges = [
            {"predicate": "hasRisk"},
            {"predicate": "leadsTo"},
        ]
        errors = ontology.validate_risk_chain_completeness(edges)
        assert any("at least one Impact" in e for e in errors)

    def test_risk_with_impact_requires_consequence(self, ontology):
        # 行为演进：impacts 不能替代 leadsTo/hasConsequence，链必须顺序完整
        edges = [
            {"predicate": "hasRisk"},
            {"predicate": "impacts"},
        ]
