...
ARBITRATION_DIR = Path(__file__).resolve().parent / "label_studio" / "arbitration" / "output"
...
def calc_level1_arbitrated() -> dict:
    ...
    by_type = defaultdict(lambda: {"tp_exact": 0, "tp_partial": 0, "fn": 0})
    total = {"tp_exact": 0, "tp_partial": 0, "fn": 0}
    ...
    for etype in ENTITY_TYPE_ORDER:
        ...
        pred_total = tp_exact + tp_partial
    ...
    for sub in STAKEHOLDER_SUBTYPES | {"Stakeholder"}:
        for k in ("tp_exact", "tp_partial", "fn"):
    ...
    results["Stakeholder(agg)"] = {
        "tp_exact": tp_exact, "tp_partial": tp_partial, "fn": fn,
...
def build_arbitrated_data(level_name: str, prefix: str, fields: list[str],
    ...
    for eid, task in original_files[1].items():
        if eid in disagree_eids and eid in arb_by_eid:
...
def print_level1(results: dict) -> None:
    print(f"\n{'Entity Type':<16} {'TP(Exact)':<10} {'TP(Partial)':<12} {'FN':<6} {'Gold':<6} {'Pred':<6} {'Precision':<10} {'Recall':<10} {'F1':<10}")
    print("-" * 96)
    for etype in ENTITY_TYPE_ORDER_DISPLAY + ["Overall"]:
        ...
        if etype == "Overall":
            print("-" * 96)
        print(f"{prefix}{etype}{suffix:<16} {r['tp_exact']:<10} {r['tp_partial']:<12} {r['fn']:<6} {r['gold_total']:<6} {r['pred_total']:<6} {p_str:<10} {rc_str:<10} {f1_str:<10}")
...
def main() -> None:
    ...
    output_path = EVAL_DIR.parent / "eval_results_arbitrated.json"
